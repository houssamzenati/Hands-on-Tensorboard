...
def mnist_model(learning_rate, use_two_fc, use_two_conv, hparam):
	...
	if use_two_fc:
		...
		embedding_size = 1024
		embedding_input = tf.nn.relu(fc1)
	...
	embedding = tf.get_variable('test_embedding', [1024, embedding_size], initializer=tf.constant_initializer(0))
	...
	for i in range(2001):
		...
		if i % 500 == 0:
			...
			saver.save(sess, os.path.join(LOGDIR, 'model.ckpt'), i) # save the model periodically
